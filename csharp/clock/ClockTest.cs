...
public class ClockTest
{
    ...
    [Test]
    public void Two_hours_are_subtracted_from_midnight()
    {
        var clock = new Clock(0, 0).Subtract(120);
        Assert.That(clock.ToString(), Is.EqualTo("22:00"));
    }

    [Test]
    public void Twenty_four_hours_are_subtracted_from_midnight()
    {
        var clock = new Clock(0, 0).Subtract(1440);
        Assert.That(clock.ToString(), Is.EqualTo("00:00"));
    }

    [Test]
    public void Forty_seven_minutes_are_subtracted()
    {
        var clock = new Clock(2, 17).Subtract(47);
        Assert.That(clock.ToString(), Is.EqualTo("01:30"));
    }

    [Test]
    public void Twenty_six_hours_are_subtracted_from_midnight()
    {
        var clock = new Clock(0, 0).Subtract(1560);
        Assert.That(clock.ToString(), Is.EqualTo("22:00"));
    }
